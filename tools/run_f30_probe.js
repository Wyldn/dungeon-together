#!/usr/bin/env node
// F30 combat competence probe — measurement only.
// Replay legitimate F30 arrivals against Vessalia and Hroth under four
// conditions. Does not retune F30, F21–29, F25, classes, potions, or F20.
//
//   node tools/run_f30_probe.js --seed 20260823 --runs 1002
//   node tools/run_f30_probe.js --seed 20260823 --runs 1002 --out reports/f30_competence.json
...
export const BOSS_IDS = {
  frost_queen: 'Queen Vessalia the Unmelting',
  tr_mon_centaur: 'Hroth, the Frost Centaur',
};

export const F30_BOSS_ORDER = ['frost_queen', 'tr_mon_centaur'];
...
export async function fightF30(run, choose) {
  ...
  const plan = planBossEncounter(rng, {
    floor: run.floor,
    boss,
    pool: ENEMIES[biome.id] || ENEMIES.frost,
    partySize: 1,
  });
  rng.advance();
...
export async function captureF30Arrival({ seed, classId, policy = 'baseline' } = {}) {
  const climbPolicy = policy === 'reasonable'
    ? (await import('./policies/reasonable.js')).reasonablePolicy()
    : baselinePolicy();
  const run = makeV2Run({ seed, classId, kitSeed: seed, name: 'F30Probe' });
  const climb = await simulateClimbV2(run, climbPolicy, { stopAfterFloor: 29 });
  const trace = climb.trace || [];
  const f20 = traceFloor(trace, 20);
  const f25 = traceFloor(trace, 25);
  const f29 = traceFloor(trace, 29);
  const segment = [21, 22, 23, 24, 25, 26, 27, 28, 29].map(fl => traceFloor(trace, fl)).filter(Boolean);

  if (climb.outcome === 'dead' || run.hp <= 0 || run.floor < 29 || !f29 || f29.outcome === 'dead') {
    return {
      reached: false,
      seed,
      classId,
  ...
  const f29LeaveHp = run.hp;
  const f29LeaveMax = run.maxHp;
  const f29Choice = f29?.meta?.choice || null;
  enterNextFloor(run);
  return {
    reached: true,
    seed,
    ...
    run,
    bossId: run.bossPicks?.[30] || null,
    arrival: snapArrival(run),
    f20: f20 ? {
      outcome: f20.outcome,
      bossId: f20.meta?.bossId || null,
      enterHpPct: f20.enter?.hpPct ?? null,
      leaveHpPct: f20.leave?.hpPct ?? null,
      won: f20.outcome !== 'dead',
    } : null,
    f25: f25 ? {
      outcome: f25.outcome,
      enterHpPct: f25.enter?.hpPct ?? null,
      leaveHpPct: f25.leave?.hpPct ?? null,
    } : null,
    f29: {
      choice: f29Choice,
      leaveHp: f29LeaveHp,
      leaveMaxHp: f29LeaveMax,
      leaveHpPct: f29LeaveHp / Math.max(1, f29LeaveMax),
      enterHpPct: f29?.enter?.hpPct ?? null,
      breathGain: run.hp - f29LeaveHp,
    },
    segment,
    trace,
  };
...
export async function replayArrival(arrival, { policy, hp, bossId } = {}) {
  ...
  if (bossId) {
    ...
    run.bossPicks[30] = bossId;
  }
  const hpEnter = run.hp;
  const hpEnterPct = run.hp / Math.max(1, run.maxHp);
  const fight = await fightF30(run, chooserFor(policy));
  return {
    policy,
    hpMode: hp,
    hpEnter,
...
function finishAgg(agg) {
  ...
  const finisherShare = specials
    .filter(([name]) => /ETERNAL WINTER|UNMELTING CHARGE/.test(name))
    .reduce((s, [, v]) => s + v, 0);
  agg.dmgShare = {
...
function finisherKillRate(g) {
  ...
  return g.topKills
    .filter(k => /ETERNAL WINTER|UNMELTING CHARGE/.test(k.name))
...
function classifyWall(byBoss, arrivals, attrition) {
  ...
  if (arrivalN < 12) {
    notes.push(`Only ${arrivalN} F30 arrivals — class/boss slices are noisy`);
    votes.push('INSUFFICIENT SAMPLE');
  }

  const avgHp = arrivals?.avgHpPct;
  if (avgHp != null && avgHp < 0.45) {
    notes.push(`F30 arrivals average ${pct(avgHp)} HP`);
    votes.push('PRE-BOSS ATTRITION');
  }
  if (attrition?.f20Winners && attrition.reachedF30 != null && attrition.f20Winners > 0) {
    const conv = attrition.reachedF30 / attrition.f20Winners;
    if (conv < 0.45) {
      notes.push(`F20 winners → F30 conversion ${pct(conv)} (${attrition.reachedF30}/${attrition.f20Winners})`);
      votes.push('PRE-BOSS ATTRITION');
    }
  }

  const vess = byBoss.frost_queen;
  const hroth = byBoss.tr_mon_centaur;
  const vD = vess?.bossaware_full;
  const hD = hroth?.bossaware_full;
  if (vD && hD && vD.n >= 12 && hD.n >= 12) {
    if (vD.winRate < 0.12 && hD.winRate >= 0.25) {
      votes.push('VESSALIA-SPECIFIC');
      notes.push(`Vessalia full+aware ${pct(vD.winRate)} vs Hroth ${pct(hD.winRate)} on identical kits`);
    } else if (hD.winRate < 0.12 && vD.winRate >= 0.25) {
      votes.push('CLASS INTERACTION');
      notes.push(`Hroth full+aware ${pct(hD.winRate)} vs Vessalia ${pct(vD.winRate)} on identical kits`);
    }
  }
  ...
  for (const [bossId, conds] of Object.entries(byBoss)) {
    ...
    if ((D.avgFreezeSkips || 0) >= 1.5) {
      notes.push(`${name}: freeze skips ${num(D.avgFreezeSkips)}/fight (applies ${num(D.avgFreezeApplies)})`);
    }
  ...
  } else if (unique.includes('VESSALIA-SPECIFIC') && unique.includes('BOSS BALANCE')) {
    primary = 'VESSALIA-SPECIFIC';
  ...
  else if (unique.includes('VESSALIA-SPECIFIC')) primary = 'VESSALIA-SPECIFIC';
  ...
  const healthyArrivals = (arrivals?.avgHpPct || 0) >= 0.45
    && (attrition?.f20Winners ? (attrition.reachedF30 / attrition.f20Winners) >= 0.45 : arrivalN >= 12);
  ...
  const justified = healthyArrivals && (bothHard || oneHard) && !policySaves;
  notes.push(justified
    ? 'F30 balance pass is justified: healthy arrivals still lose at full HP under competent play.'
    : 'F30 balance pass is not clearly justified from this matrix (attrition, policy, or a viable boss remains).');
...
function theoreticalPressure(bossId) {
  const floor = 30;
  const spec = bossId === 'tr_mon_centaur' ? ALT_BOSSES[30] : BOSSES[30];
  if (!spec) return null;
  ...
  return {
    ...
    freezeEvery: spec.freezeEvery || null,
    freezeOnHit: spec.freeze || null,
    scaleHp: sc.hp,
    scaleAtk: sc.atk,
    ...
    specials: specials.map(s => ({
      ...
      freeze: s.freeze || null, freezeSure: !!s.freezeSure, weaken: s.weaken || null, aoe: !!s.aoe,
    })),
    basicRaw: Math.round(basicRaw),
    midSpecial: mid ? { name: mid.name, at: mid.at, mult: mid.mult } : null,
    finisher: finisher ? { name: finisher.name, at: finisher.at, mult: finisher.mult, freezeSure: !!finisher.freezeSure } : null,
  };
}
...
function estimateHitVsArrival(pressure, arrival, special) {
  ...
  const banked = special ? soloBossChargeForScale(30, special.at || 0) : 0;
...
export function formatProbeReport(rep) {
  const lines = [];
  lines.push(`DungeonTogether F30 combat competence — seed ${rep.meta.seed}  climbs ${rep.meta.climbs}  arrivals ${rep.meta.arrivals}`);
  lines.push(`Capture policy: ${rep.meta.capturePolicy}  each arrival replayed vs both bosses × 4 conditions`);
  lines.push(`Climb RNG isolated from combat policy; F30 authored kits unchanged`);
  lines.push('');

  if (rep.arrivals) {
    const a = rep.arrivals;
    lines.push('=== F30 arrival health ===');
    ...
    if (a.f20Wins != null) lines.push(`  F20 wins among arrivals: ${a.f20Wins}/${a.n}`);
    if (a.f29Choices) {
      lines.push(`  F29 campfire: ${Object.entries(a.f29Choices).map(([k, n]) => `${k} ${n}`).join(', ') || 'none'}`);
    }
    if (a.liveBoss) {
      lines.push(`  live assignment: ${Object.entries(a.liveBoss).map(([id, n]) => `${BOSS_IDS[id] || id} ${n}`).join(', ')}`);
    }
    if (a.byLiveBoss) {
      for (const id of F30_BOSS_ORDER) {
  ...
  if (rep.compareF20) {
    const c = rep.compareF20;
    lines.push('=== F20 vs F30 arrival comparison (same climbs) ===');
    lines.push(`  F20 winners          n=${c.f20Winners?.n || 0}  HP ${pct(c.f20Winners?.avgHpPct)}  potions ${num(c.f20Winners?.avgPotions)}  relics ${num(c.f20Winners?.avgRelics)}`);
    lines.push(`  F30-bound at F20     n=${c.f30AtF20?.n || 0}  HP ${pct(c.f30AtF20?.avgHpPct)}  potions ${num(c.f30AtF20?.avgPotions)}  relics ${num(c.f30AtF20?.avgRelics)}`);
    lines.push(`  F30 arrivals         n=${c.f30?.n || 0}  HP ${pct(c.f30?.avgHpPct)}  potions ${num(c.f30?.avgPotions)}  relics ${num(c.f30?.avgRelics)}`);
    lines.push('');
  }

  for (const bossId of F30_BOSS_ORDER) {
    ...
    for (const [label, g] of rows) {
      ...
      lines.push(`      freeze apply ${num(g.avgFreezeApplies)}  skip ${num(g.avgFreezeSkips)}`);
      ...
      if (g.dmgShare) {
        ...
        lines.push(`      dmg: basic ${pct(g.dmgShare.basic)}  finisher ${pct(g.dmgShare.finisher)}  escort ${pct(g.dmgShare.escort)}  ${spec}`);
      }
    }
    lines.push('');
  ...
  if (rep.liveByBoss) {
    lines.push('=== Live assignment (natural F30 pick, not cross-replay) ===');
    for (const bossId of F30_BOSS_ORDER) {
  ...
  if (rep.pressure) {
    lines.push('=== Fight-length / pressure (authored scale, F30 solo uncapped charge) ===');
    for (const bossId of F30_BOSS_ORDER) {
      const p = rep.pressure[bossId];
      if (!p) continue;
      lines.push(`  ${p.name}: eHP ${p.ehp}  ATK ${p.eatk}  SPD ${p.authoredSpd}  basic≈${p.basicRaw}  freezeEvery ${p.freezeEvery || '—'}  `
  ...
  if (rep.attrition) {
    ...
    lines.push('=== F20 → F30 attrition ===');
    lines.push(`  reached F20 ${a.reachedF20}  F20 deaths ${a.f20Deaths}  F20 winners ${a.f20Winners}  F20 survival ${pct(a.f20Survival)}`);
    lines.push(`  F20 winners → F30 ${a.reachedF30} (${pct(a.f20Winners ? a.reachedF30 / a.f20Winners : 0)})`);
    if (a.deathFloors) {
      lines.push(`  deaths after F20 win: ${Object.entries(a.deathFloors).map(([f, n]) => `F${f} ${n}`).join('  ') || 'none'}`);
    }
    if (a.floors) {
      for (const fl of [21, 22, 23, 24, 25, 26, 27, 28, 29]) {
        const b = a.floors[fl];
        if (!b) continue;
    ...
    if (a.f29Choices) {
      lines.push(`  F29 campfire (F20 winners who arrived): ${Object.entries(a.f29Choices).map(([k, n]) => `${k} ${n}`).join(', ') || 'none'}`);
    }
    lines.push('');
  }

  if (rep.representatives?.length) {
    lines.push('=== Representative F30 arrivals (median HP% per class) ===');
    for (const r of rep.representatives) {
      lines.push(`  ${r.classId.padEnd(12)} seed ${r.seed}  live ${BOSS_IDS[r.bossId] || r.bossId}  `
        + `HP ${r.hp}/${r.maxHp} (${pct(r.hpPct)})  potions ${r.potions}  relics ${r.relics}  skills ${r.skills}  F20 ${r.f20Boss || '—'}`);
    }
    lines.push('');
  }
  ...
  lines.push(`  F30 balance pass justified: ${rep.classification.balancePassJustified ? 'YES' : 'NO'}`);
...
export async function runF30Probe({
  seed = 20260823,
  runs = 1002,
  ...
  const byBoss = {
    frost_queen: emptyCondSet(),
    tr_mon_centaur: emptyCondSet(),
  };
  const liveByBoss = {};
  const arrivalHealth = emptyHealth();
  const arrivalByLiveBoss = {
    frost_queen: emptyHealth(),
    tr_mon_centaur: emptyHealth(),
  };
  const f20WinnersHealth = emptyHealth();
  const f30AtF20 = emptyHealth();
  const f29Choices = {};
  const liveBossCounts = {};
  const arrivalF20Wins = { n: 0, wins: 0 };
  const attrition = {
    reachedF20: 0,
    f20Deaths: 0,
    f20Winners: 0,
    ...
    deathFloors: {},
    floors: {
      21: emptyFloorBand(), 22: emptyFloorBand(), 23: emptyFloorBand(), 24: emptyFloorBand(),
      25: emptyFloorBand(), 26: emptyFloorBand(), 27: emptyFloorBand(), 28: emptyFloorBand(),
      29: emptyFloorBand(),
    },
    f29Choices: {},
  };
  ...
  for (const job of jobs) {
    climbs += 1;
    const cap = await captureF30Arrival({ seed: job.seed, classId: job.classId, policy: capturePolicy });

    const rec20 = traceFloor(cap.trace, 20);
    const reachedF20 = !!rec20;
    if (reachedF20) {
      attrition.reachedF20 += 1;
      const won20 = rec20.outcome !== 'dead';
      if (!won20) attrition.f20Deaths += 1;
      else {
        attrition.f20Winners += 1;
        if (rec20.leave) pushHealth(f20WinnersHealth, rec20.leave, cap.classId);
        for (const fl of [21, 22, 23, 24, 25, 26, 27, 28, 29]) {
        ...
        if (!cap.reached) {
          ...
          if (df >= 21 && df <= 30) attrition.deathFloors[df] = (attrition.deathFloors[df] || 0) + 1;
        }
        const rec29 = traceFloor(cap.trace, 29);
        if (rec29?.meta?.choice) {
          attrition.f29Choices[rec29.meta.choice] = (attrition.f29Choices[rec29.meta.choice] || 0) + 1;
        }
      }
    }
    ...
    const liveId = cap.bossId || cap.run.bossPicks?.[30] || 'unknown';
    ...
    if (cap.f20?.won) {
      const rec20Leave = traceFloor(cap.trace, 20);
      if (rec20Leave?.leave) pushHealth(f30AtF20, rec20Leave.leave, cap.classId);
    }
    if (cap.f29?.choice) f29Choices[cap.f29.choice] = (f29Choices[cap.f29.choice] || 0) + 1;
    if (cap.f20) {
      arrivalF20Wins.n += 1;
      if (cap.f20.won) arrivalF20Wins.wins += 1;
    }
    liveArrivals.push({
      seed: cap.seed,
      classId: cap.classId,
      ...
      f20Boss: cap.f20?.bossId || null,
    });

    for (const cond of CONDITIONS) {
      for (const otherId of F30_BOSS_ORDER) {
  ...
  arrivalsOut.f29Choices = f29Choices;
  arrivalsOut.f20Wins = arrivalF20Wins.wins;
  arrivalsOut.liveBoss = liveBossCounts;
  arrivalsOut.byLiveBoss = {
    frost_queen: finishHealth(arrivalByLiveBoss.frost_queen),
    tr_mon_centaur: finishHealth(arrivalByLiveBoss.tr_mon_centaur),
  };
  ...
  const medianCap = medianArrival
    ? await captureF30Arrival({ seed: medianArrival.seed, classId: medianArrival.classId, policy: capturePolicy })
    : null;
  for (const bossId of F30_BOSS_ORDER) {
  ...
  attrition.f20Survival = attrition.reachedF20 ? (attrition.f20Winners / attrition.reachedF20) : 0;
  ...
  return {
    ...
    compareF20: {
      f20Winners: finishHealth(f20WinnersHealth),
      f30AtF20: finishHealth(f30AtF20),
      f30: arrivalsOut,
    },
    byBoss,
    liveByBoss,
...
async function main() {
  ...
  const out = flags.out || 'reports/f30_competence.json';
  const rep = await runF30Probe({ seed, runs, capturePolicy, classId });
...
const isMain = process.argv[1] && process.argv[1].replace(/\\/g, '/').endsWith('tools/run_f30_probe.js');
if (isMain) main();
